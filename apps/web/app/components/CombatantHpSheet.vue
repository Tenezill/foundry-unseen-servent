<template>
  <div class="scrim" @click.self="emit('close')">
    <div class="modal-sheet" role="dialog" aria-modal="true" :aria-label="`Apply damage or healing to ${combatant.name}`">
      <div class="head">
        <span class="title">{{ combatant.name }}</span>
        <span class="current">
          <template v-if="combatant.isPC && combatant.hp">
            {{ combatant.hp.value }}<span class="max">/{{ combatant.hp.max }}</span>
          </template>
          <template v-else-if="combatant.health">{{ healthLabel }}</template>
        </span>
      </div>
      ...
      <div class="actions">
        <button class="act damage" type="button" :disabled="amount === 0 || busy" @click="apply(-1)">
          &minus; Damage
        </button>
        <button class="act heal" type="button" :disabled="amount === 0 || busy" @click="apply(1)">
          + Heal
        </button>
      </div>
...
const props = defineProps<{ combatant: EncounterCombatantView; busy?: boolean }>()
...
const healthLabel = computed(() => {
  const h = props.combatant.health
